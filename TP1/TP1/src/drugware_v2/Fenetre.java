...
public class Fenetre extends JFrame {
	...
	public Fenetre() {
		...
		this.setTitle("Bienvenue à la pharmacie du Dr.Prabhjot.");
		...
		//Les fichiers sont charger au début de l'aplication.
		...
		itemPresc3 = new JMenuItem("Entrer une nouvelle prescription");
	...
	public class BoutonPresc3Listener implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			...
			if(pharma.siClientExiste(NAM)){
				...
				do{
					nomMarque = JOptionPane.showInputDialog(null,
							"Entre le nom du médicament", "Prescription",
							JOptionPane.QUESTION_MESSAGE);
				}while(nomMarque == null);


				do{
				doses = JOptionPane.showInputDialog(null,
							"Entre le niveau de dosages",
							"Prescription du medicament",
							JOptionPane.QUESTION_MESSAGE);
				}while(validerDouble(doses).equalsIgnoreCase("0"));
				
				do{
				renouvellement = JOptionPane.showInputDialog(null,
							"Entre le nombre de renouvellement du medicament",
							"Prescription",
							JOptionPane.QUESTION_MESSAGE);
				}while(validerDouble(renouvellement).equalsIgnoreCase("0"));

				Prescription prescription = new Prescription();
				prescription.creeNouvPrescription(nomMarque,Double.parseDouble(doses),Integer.parseInt(renouvellement),liste);
			} else {
				JOptionPane.showMessageDialog(null,
						"NAM invalide, client introuvable.",
						"Client introuvable", JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
	
	public String validerDouble(String valeur){
			if(valeur == null){
				valeur = "0";
			}else if(! valeur.matches("[0-9]+")){
				valeur = "0";
				JOptionPane.showMessageDialog(null, "Veuillez entré un nombre.");
			}
		return valeur;
	}
